...
class PortfolioAnalytics:
    ...
    Handles NaN values in exposures, weights, and specific variances by
    identifying valid (non-NaN) stocks and working only with that subset.
    Weights are renormalized over the valid subset to preserve portfolio
    scale (e.g. fully-invested sums to 1.0).
    """
    ...
    def __init__(self, data: FactorModelData, model: FactorModelResults):
        ...
        w_p = np.asarray(data.portfolio_weights, dtype=float)
        w_b = np.asarray(data.benchmark_weights, dtype=float)
        X_raw = data.factor_exposures.values.astype(float)    # N × K
        sv_raw = model.specific_variance.values.astype(float) # N

        # --- Identify valid stocks: non-NaN in weights, exposures, specific var ---
        valid = (~np.isnan(w_p)
                 & ~np.isnan(w_b)
                 & ~np.isnan(X_raw).any(axis=1)
                 & ~np.isnan(sv_raw))

        self._valid = valid
        self.n_valid = int(valid.sum())
        self.n_dropped = int(len(valid) - self.n_valid)

        # Subset and renormalize weights
        self.w_p = self._renorm(w_p, valid)
        self.w_b = self._renorm(w_b, valid)
        self.w_a = self.w_p - self.w_b          # active weights

        self.X = X_raw[valid]                    # n_valid × K
        self.F = model.factor_covariance.values  # K × K
        self.Delta = np.diag(sv_raw[valid])      # n_valid × n_valid

        # Recompute V for the valid subset: V = X F X' + Δ
        self.V = self.X @ self.F @ self.X.T + self.Delta  # n_valid × n_valid

        # Keep valid asset names for labelling
        self._valid_asset_names = [
            n for n, v in zip(data.asset_names, valid) if v
        ]

    @staticmethod
    def _renorm(weights: np.ndarray, valid: np.ndarray) -> np.ndarray:
        """Subset weights to valid stocks and rescale to preserve the original sum."""
        w = weights[valid].copy()
        orig_sum = np.nansum(weights)
        new_sum = w.sum()
        if abs(orig_sum) > 1e-10 and abs(new_sum) > 1e-10:
            w *= orig_sum / new_sum
        return w
    ...
    def risk_decomposition(self) -> pd.Series:
        ...
        return pd.Series({
            ...
            "N Valid Stocks": self.n_valid,
            "N Dropped Stocks": self.n_dropped,
        })
    ...
    def factor_risk_contribution(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame({
            ...
            "% of Systematic Var": contributions / sys_var * 100 if sys_var != 0 else contributions * 0,
    ...
    def mctr_total(self) -> pd.Series:
        ...
        return pd.Series(Vw / sigma, index=self._valid_asset_names, name="MCTR")

    def mctr_active(self) -> pd.Series:
        """MCTR_i (active) = (V w_a)_i / ω"""
        Vw = self.V @ self.w_a
        te = self.tracking_error()
        return pd.Series(Vw / te, index=self._valid_asset_names, name="Active MCTR")
    ...
    def transfer_coefficient(self, alpha_scores: Optional[np.ndarray] = None) -> float:
        ...
        if alpha_scores is None:
            # Use factor-model-implied alphas (valid subset only)
            factor_premiums = self.model.factor_returns.mean().values
